...
def generate_yaml(dumps_path, dataset_path, experiments_name, dataset, num_nodes_list, compressor_to_step_size_range, number_of_seeds,
                  ...
                  no_initial_mega_batch_size_zero, momentums, split_into_groups_by_labels, task,
                  quad_dim, quad_noise_lambda, quad_strongly_convex_constant, quad_noise, encode_dim, ef21_double_momentum,
                  early_stop_function):
        ...
        for gamma_multiply in [2**i for i in range(step_size_range[0], step_size_range[1])]:
            ...
            for momentum in momentums_loop:
                ...
                if yaml_prepared['task'] == 'libsvm':
                    ...
                    yaml_prepared['noise'] = quad_noise
                else:
                    assert False
                ...
                if algorithm_name == 'ef21_stochastic':
                    yaml_prepared['algorithm_node_params']['double_momentum'] = ef21_double_momentum
...
if __name__ == "__main__":
    ...
    parser.add_argument('--momentums', required=True, nargs='+', type=float)
    ...
    parser.add_argument('--ef21_double_momentum', action='store_true')
    ...
    parser.add_argument('--early_stop_function', type=float, default=None)
    ...
    generate_yaml(args.dumps_path, args.dataset_path, args.experiments_name, args.dataset, args.num_nodes_list, compressor_to_step_size_range,
                  ...
                  args.momentums, args.split_into_groups_by_labels, args.task,
                  args.quad_dim, args.quad_noise_lambda, args.quad_strongly_convex_constant,
                  args.quad_noise, args.encode_dim, args.ef21_double_momentum, args.early_stop_function)
