...
singularity_template = 'CUDA_VISIBLE_DEVICES={device} PYTHONPATH=./code python3.7 code/distributed_optimization_library/experiments/experiments.py --path_to_dataset {dataset_path} --dumps_path {dumps_path}/{experiments_name}/ --config {config_name}'
...
def generate_yaml(dumps_path, dataset_path, experiments_name, num_nodes_list, step_size_range,
                  ...
                  mega_batch_sizes, calculate_accuracy):
    ...
    for _, (compressor_name, algorithm_name, num_nodes) in enumerate(
        ...
        if algorithm_name in ['zero_marina_stochastic', 'zero_marina_partial_participation_stochastic']:
            extra_parameters = noise_momentums
        elif algorithm_name in ['ef21_stochastic', 'ef_stochastic']:
            extra_parameters = noise_momentums
        elif algorithm_name == 'zero_marina_sync_stochastic':
            extra_parameters = mega_batch_sizes
        elif algorithm_name in ['marina_stochastic', 'marina_stochastic_partial_participation']:
            extra_parameters = mega_batch_sizes
        elif algorithm_name in ['stochastic_gradient_descent', 'frecon_stochastic']:
        ...
        for gamma in step_size_range:
            for extra_parameter in extra_parameters:
                ...
                if algorithm_name == 'ef21_stochastic':
                    yaml_prepared['algorithm_node_params'] = {}
                    yaml_prepared['algorithm_node_params']['momentum'] = extra_parameter
                    yaml_prepared['algorithm_master_params']['init_with_gradients'] = True
                    yaml_prepared['compressor_name'] = compressor_name
                    yaml_prepared['compressor_params'] = {'is_cuda': True, 
                                                          'number_of_coordinates': number_of_coordinates}
                if algorithm_name == 'frecon_stochastic':
                    yaml_prepared['algorithm_master_params']['initial_mega_batch_size'] = 1
                    yaml_prepared['algorithm_master_params']['number_of_samples'] = 1
                    yaml_prepared['compressor_name'] = compressor_name
                    yaml_prepared['compressor_params'] = {'is_cuda': True, 
                                                          'number_of_coordinates': number_of_coordinates}
                if algorithm_name == 'zero_marina_partial_participation_stochastic':
                    yaml_prepared['algorithm_master_params']['noise_momentum'] = extra_parameter
                    yaml_prepared['algorithm_master_params']['initial_mega_batch_size'] = 1
                    yaml_prepared['algorithm_master_params']['number_of_samples'] = 1
                    yaml_prepared['compressor_name'] = compressor_name
                    yaml_prepared['compressor_params'] = {'is_cuda': True, 
                                                          'number_of_coordinates': number_of_coordinates}
                if algorithm_name == 'marina_stochastic_partial_participation':
                    ...
                    yaml_prepared['compressor_params'] = {'is_cuda': True, 
                                                          'number_of_coordinates': number_of_coordinates}
                if algorithm_name == 'ef_stochastic':
                    yaml_prepared['compressor_name'] = compressor_name
                    yaml_prepared['compressor_params'] = {'is_cuda': True, 
                                                          'number_of_coordinates': number_of_coordinates}
...
if __name__ == "__main__":
    ...
    generate_yaml(args.dumps_path, args.dataset_path, args.experiments_name, args.num_nodes_list, args.step_size_range,
                  ...
                  args.mega_batch_sizes, args.calculate_accuracy)
